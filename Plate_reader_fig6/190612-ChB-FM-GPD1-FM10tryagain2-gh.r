...
#chromesome X CRISPR swap fluorescence (data from 2019-06-12)
...
all1 = list.files(path=paste(getwd(),"/",f1,sep = ""), "190612-.*.txt", full = TRUE)
...
pdf(file = "190612-ChB-FM-GPD1-FM10tryagain2.pdf",width = 10,height = 9)
genes<-c("FM10a","FM10b","FM10c","FM10d","MKT1")
...
for (j in 1:5) {
  ...
  for (i in levels(as.factor(datacfsT$uniquePos))) {
    if (length(grep(pattern = "_d_",x = i))>0) {tempcol<-"grey"}
...
for (j in 1:5) {
  datacfsT<-datacfs1[datacfs1$cond==genes[j],]
  levels(as.factor(datacfsT$uniquePos))
  ...
  plot(0,0,xlim=c(0,18),ylim=c(-4,1),type='n',main=genes[j],xlab="time (h)", ylab="GFP (ratio)")
  for (i in 1:3) {
    datacfsT<-datacfs1[datacfs1$cond==genes[j] & datacfs1$cond2==typ[i],]
    tempcol<-colt[i]
    tempcol2<-colt2[i]
    x<-datacfsT$time/3600
    y<-log(datacfsT$SmOD)
...
values<-values[values$Version!="d",]
...
#now ploting (boxplot)
values3<-values2[values2$Genetested!="MKT1",]
values3$colplot<-rep("#0000ffff",nrow(values3))
values3$colplot[values3$Version=="RM"]<-"#e3d905ff"
values3$test<-as.factor(as.character(values3$test))
limHIGH<-10000
svg(filename = "chrXswaptest1.svg",width = 6,height = 4.5)
boxplot(values3$GFPuseR~values3$test,pch=1,cex=0.4,col="#00CC80",ylim = c(2000,limHIGH))
...
region<-levels(values3$Genetested)[c(3,4,5,6)]

i=region[1]
for (i in region) {
  test<-t.test(values3$GFPuseR[values3$Genetested==i & values3$Version == "BY"],values3$GFPuseR[values3$Genetested==i & values3$Version == "RM"])
  print(paste(log2(test$estimate[2]/test$estimate[1]),test$p.value))
}

test<-t.test(values3$GFPuseR[values3$Genetested!=region[1]],values3$GFPuseR[values3$Genetested==region[1]])
print(paste(log2(test$estimate[2]/test$estimate[1]),test$p.value))

#same with log2
i=region[1]
for (i in region) {
  test<-t.test(log2(values3$GFPuseR[values3$Genetested==i & values3$Version == "BY"]),log2(values3$GFPuseR[values3$Genetested==i & values3$Version == "RM"]))
  print(paste(test$estimate[2]-test$estimate[1],test$p.value))
}

test<-t.test(log2(values3$GFPuseR[values3$Genetested!=region[1]]),log2(values3$GFPuseR[values3$Genetested==region[1]]))
print(paste(test$estimate[2]-test$estimate[1],test$p.value))