...
#Z3EV estradiol test with replicative plasmid carrying dCAS9 (data from 2017-06-29)
...
all1 = list.files(path=paste(getwd(),"/",f1,sep = ""), "170629-.*.txt", full = TRUE)
...
datacfs1_row<-9-as.numeric(factor(datacfs1$row,levels=c("A","B","C","D","E","F","G","H")))
plot(0,0,xlim=c(0,30*12),ylim=c(0,0.5*8),type = "n",xlab = "",ylab = "OD",yaxt="n",main = "OD")
for (id in levels(as.factor(datacfs1$uniquePos))) {
  points(datacfs1$time[datacfs1$uniquePos == id]/3600+30*(datacfs1$col[datacfs1$uniquePos == id][1]-1),datacfs1$SmOD[datacfs1$uniquePos == id]+5+0.5*(datacfs1_row[datacfs1$uniquePos == id][1]-11),type = "l",col = "black")
}
...
valuesCAS9<-values[values$Background=="BY_wt" & values$Genetested %in% c("empty","CAS9"),]
valuesCAS9m<-valuesCAS9[valuesCAS9$Clone==2,]
...
pl<-barplot(valuesCAS9m$GFPuseR,col="#34ab83ff")
well<-c()
xval<-c()
time<-c()
OD<-c()
GFP<-c()
mCH<-c()
for (i in 1:nrow(valuesCAS9)) {
  ODv<-datacfs1$SmOD[datacfs1$uniquePos==valuesCAS9$wells[i]]
  pltemp<-pl[valuesCAS9m$Report==valuesCAS9$Report[i] & valuesCAS9m$Genetested==valuesCAS9$Genetested[i] & valuesCAS9m$as.numeric.Version.==valuesCAS9$as.numeric.Version.[i]]
  xvaltemp<-pltemp+(as.numeric(as.character(valuesCAS9$Clone[i]))-1.5)/3
  xval<-c(xval,rep(xvaltemp,5))
  well<-c(well,rep(as.character(valuesCAS9$wells[i]),5))
  time<-c(time,datacfs1$time[datacfs1$uniquePos==valuesCAS9$wells[i]][rowIndicesUser(ODv)])
  OD<-c(OD,datacfs1$value[datacfs1$uniquePos==valuesCAS9$wells[i]][rowIndicesUser(ODv)])
  GFP<-c(GFP,datacfs1$GFP[datacfs1$uniquePos==valuesCAS9$wells[i]][rowIndicesUser(ODv)])
  mCH<-c(mCH,datacfs1$mCH[datacfs1$uniquePos==valuesCAS9$wells[i]][rowIndicesUser(ODv)])
  plot(datacfs1$time[datacfs1$uniquePos==valuesCAS9$wells[i]],datacfs1$value[datacfs1$uniquePos==valuesCAS9$wells[i]])
  points(datacfs1$time[datacfs1$uniquePos==valuesCAS9$wells[i]][rowIndicesUser(ODv)],datacfs1$value[datacfs1$uniquePos==valuesCAS9$wells[i]][rowIndicesUser(ODv)],pch=16,col="black")
}
...
svg(filename = "plasmCAS9_GFP.svg",width = 7,height = 3.7)
barplot(valuesCAS9m$GFPuseR,col="#34ab83ff",ylim=c(0,100000))
points(pointvalue$xval,pointvalue$GFPr,pch=16,cex=1)
dev.off()

svg(filename = "plasmCAS9_mCH.svg",width = 7,height = 3.7)
barplot(valuesCAS9m$mCHuseR,ylim = c(0,3000),col="red")
points(pointvalue$xval,pointvalue$mCHr,pch=16,cex=1)
dev.off()
